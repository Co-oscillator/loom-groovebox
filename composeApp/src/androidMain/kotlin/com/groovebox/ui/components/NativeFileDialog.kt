...
actual fun NativeFileDialog(
    ...
    val files: List<String> = remember(currentDir, refreshKey, currentExtensions) { 
        val list = if (!currentDir.exists() || !currentDir.isDirectory) {
             emptyList()
        } else {
        ...
        if (currentDir.name == "wavetables" && !isSave) {
            listOf("[ BASIC WAVETABLE ]") + list
        } else list
    }
    ...
    Dialog(
        ...
        Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
            ...
            Column(
                ...
                val headerText = (title) + " (${files.size} files)"
                
                Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.SpaceBetween) {
                    Text(
                        headerText,
                        style = MaterialTheme.typography.titleMedium,
                        color = getEngineColor(if (trackIndex != -1) state.tracks[trackIndex].engineType else EngineType.SAMPLER)
                    )
                ...
                Row(
                    ...
                    loomFolders.forEach { folderName ->
                        val rootDir = PersistenceManager.getLoomFolder()
                ...
                if (!isSave) {
                    Button(
                        onClick = { 
                           ...
                           val mime = if (extensions.contains("sf2")) "*/*" else "audio/*"
                ...
                if (isSave) {
                    ...
                    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                        Button(
                            ...
                            val buttonText = if (title.contains("SAMPLE", true)) "SAVE AS NEW SAMPLE FILE" else "SAVE NEW PRESET FILE"
                            Text(buttonText)
                        }
                            ...
                            Box(modifier = Modifier.fillMaxWidth()) {
                                Row(
                                    modifier = Modifier
                                        ...
                                        .combinedClickable(
                                            onClick = {
                                                if (name == "[ BASIC WAVETABLE ]") {
                                                    onFileSelected("DEFAULT")
                                                    onDismiss()
                                                } else if (isSave) fileName = name.removeSuffix(".wav")
                                                else {
                                                    onFileSelected(file.absolutePath)
                                                    onDismiss()
                                                }
